...
import type { AppState, Riad, Estimation, Prestataire, Rdv, ZonesSurfaces } from '@/types'
import { loadFromDB, saveRiad, deleteRiad as dbDeleteRiad, savePrestataire, deletePrestataire as dbDeletePresta, saveEstimation, saveRdv, deleteRdv as dbDeleteRdv } from './db'

const LS_KEY = 'riad-vision-v6'
...
const DEFAULT_STATE: AppState = {
  riads: DEMO_RIADS, estimation: { riadId: null, mode: 'rapide', niveau: 'complete', surface: 200, zones: { ...DEFAULT_ZONES }, transf: [], prixPerso: '' },
  prestataires: [], rdvs: [], nextId: 4, nextPrestaId: 1, nextRdvId: 1,
}
...
export function useAppState() {
  ...
  const addRiad = useCallback((riad: Omit<Riad, 'id' | 'createdAt'>) => {
    setState(s => { const r: Riad = { ...riad, id: s.nextId, createdAt: new Date().toISOString() }; setTimeout(() => saveRiad(r), 0); return { ...s, riads: [...s.riads, r], nextId: s.nextId + 1 } })
  }, [])
  ...
  return { state, loaded, addRiad, updateRiad, deleteRiad, setEstimation, addPrestataire, updatePrestataire, deletePrestataire, addRdv, updateRdv, deleteRdv }
}
